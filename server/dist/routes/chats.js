...
router.get('/user/:userId', async (req, res) => {
    try {
        const chats = await Chat_1.default.find({ members: req.params.userId })
...
router.post('/', async (req, res) => {
    try {
        ...
        if (userId && otherUserId) {
            ...
            const user1 = await User_1.default.findById(userId);
            const user2 = await User_1.default.findById(otherUserId);
            if (!user1 || !user2) {
                return res.status(404).json({ message: 'User not found' });
            }
            const newPrivateChat = new Chat_1.default({
...
router.get('/:chatId/messages', async (req, res) => {
    try {
        const messages = await Message_1.default.find({
            ...
            // .populate('sender', 'name avatar email') // Removed: sender is now String
            // .populate('receiver', 'name avatar email') // Removed: receiver is now String
            .sort({ timestamp: 1 });
        res.json(messages);
    }
    catch (error) {
...
router.put('/:chatId', async (req, res) => {
    try {
        ...
        const chat = await Chat_1.default.findByIdAndUpdate(req.params.chatId, { disappearingMessagesDuration }, { new: true });
        res.json(chat);
    }
    catch (error) {
    ...
    try {
        ...
        if (chat && chat.type === 'personal') {
            ...
            if (otherMember) {
                ...
                const receiverUser = await User_1.default.findById(receiverId);
                if (receiverUser)
                    receiverName = receiverUser.name;
            }
        }
        const senderUser = await User_1.default.findById(sender);
        if (senderUser) {
        ...
        // await savedMessage.populate('sender', 'name avatar email'); // No longer needed
        await Chat_1.default.findByIdAndUpdate(req.params.chatId, {
            lastMessage: {
                text: text,
...
router.post('/groups', async (req, res) => {
    try {
        ...
        const newGroup = new Chat_1.default({
            name,
            type: 'group',
            members,
...
router.delete('/:chatId', async (req, res) => {
    try {
        const chat = await Chat_1.default.findOneAndDelete({ _id: req.params.chatId });
        if (!chat) {
            return res.status(404).json({ message: 'Chat not found' });
        }
        // Also delete associated messages
        await Message_1.default.deleteMany({ chatId: req.params.chatId });
...
router.delete('/:chatId/members/:userId', async (req, res) => {
    try {
        ...
        // If no members left, maybe delete chat? For now, just save.
        await chat.save();
