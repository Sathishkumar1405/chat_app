...
router.put('/:chatId/messages/:messageId/star', async (req: Request, res: Response) => {
    try {
...
router.post('/:chatId/messages', async (req: Request, res: Response) => {
    try {
        ...
        if (chat && chat.type === 'personal') {
            const otherMember = chat.members.find((m: any) => m.toString() !== sender);
            if (otherMember) {
...
router.delete('/:chatId/members/:userId', async (req: Request, res: Response) => {
    try {
        ...
        // Remove the member
        chat.members = chat.members.filter((memberId: any) => memberId.toString() !== userId);
