...
const seedDatabase = async () => {
  try {
    ...
    const sportsChannel = await Chat.findOne({ name: 'General', type: 'channel' });
    if (sportsChannel) {
      const richMsgCount = await Message.countDocuments({ chatId: sportsChannel._id, type: 'channel_post' });
      if (richMsgCount < 50) {
